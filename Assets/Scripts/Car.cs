...
public class Car : MonoBehaviour
{
    ...
    public float previousOffsetAngleFromGroundOnLanding { get; private set; }

    public GearShiftMode GearShiftMode { get => gearShiftMode; set => gearShiftMode = value; }
    ...
    private void ComputeGroundAndAirStatus()
    {
        ...
        {
            float timeInAirBeforeCol = timeInAir;
            lastContactWithGround = 0;
            timeInAir = 0;
            triggeredGroundLeftAlready = false;
            justLeftGround = false;

            //this if(){} is just for testing purpose, please ignore
            if (timeInAirBeforeCol > 0.5f)
            {
                previousOffsetAngleFromGroundOnLanding = Vector3.Angle(previouslyAcceptedGroundNormal, transform.up);
            }
        }
        else
        {
